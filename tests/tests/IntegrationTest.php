...
class IntegrationTest extends TestCase
{
     ...
     */
    private function getTracker($type): Tracker
    {
        $subject = new Subject();
    ...
    private function getSocketEmitter($type): SocketEmitter
    {
     ...
     * @throws ErrorException
     */
    private function getCurlEmitter($type): CurlEmitter
    {
    ...
    private function getUnstructEvent(): array
    {
        return [
     ...
     * @throws ErrorException
     */
    public function testPvGet(): void
    {
     ...
     * @throws ErrorException
     */
    public function testPvPost(): void
    {
     ...
     * @throws ErrorException
     */
    public function testSvGet(): void
    {
        $tracker = $this->getTracker('GET');
     ...
     * @throws ErrorException
     */
    public function testSvPost(): void
    {
        $tracker = $this->getTracker('POST');
        $tracker->trackScreenView('HUD', 'Level: 23', $this->getContext());
        $tracker->flushEmitters();
     ...
     * @throws ErrorException
     */
    public function testStructGet(): void
    {
        $tracker = $this->getTracker('GET');
     ...
     * @throws ErrorException
     */
    public function testStructPost(): void
    {
        $tracker = $this->getTracker('POST');
        $tracker->trackStructEvent('shop', 'add-to-basket', null, 'pcs', 2, $this->getContext());
        $tracker->flushEmitters();
     ...
     * @throws ErrorException
     */
    public function testUnstructGet(): void
    {
        $tracker = $this->getTracker('GET');
     ...
     * @throws ErrorException
     */
    public function testUnstructPost(): void
    {
        $tracker = $this->getTracker('POST');
        $tracker->trackUnstructEvent($this->getUnstructEvent(), $this->getContext());
        $tracker->flushEmitters();
     ...
     * @throws ErrorException
     */
    public function testEcommerceGet(): void
    {
        $tracker = $this->getTracker('GET');
     ...
     * @throws ErrorException
     */
    public function testEcommercePost(): void
    {
        $tracker = $this->getTracker('POST');
        $tracker->trackEcommerceTransaction(
            'test_order_id_1',
     ...
     * @throws ErrorException
     */
    public function testDebugSwitch(): void
    {
        $tracker = $this->getTracker('GET');
        $tracker->trackPageView('www.example.com', 'example', 'www.referrer.com');
        $tracker->flushEmitters();
    ...
    private function requestResultAssert($emitters, $test): void
    {
        foreach ($emitters as $emitter) {
            $results = $emitter->returnRequestResults();
            ...
            foreach ($results as $result) {
                switch ($test) {
                    case 'pv-get' :
                        $this->assertPageViewGet($result['data']);

                        break;
                    case 'pv-post' :
                        $this->assertPageViewPost($result['data']);

                        break;
                    case 'sv-get' :
                        $this->assertScreenViewGet($result['data']);

                        break;
                    case 'sv-post' :
                        $this->assertScreenViewPost($result['data']);

                        break;
                    case 'struct-get' :
                        $this->assertStructEventGet($result['data']);

                        break;
                    case 'struct-post' :
                        $this->assertStructEventPost($result['data']);

                        break;
                    case 'unstruct-get' :
                        $this->assertUnStructEventGet($result['data']);

                        break;
                    case 'unstruct-post' :
                        $this->assertUnStructEventPost($result['data']);

                        break;
                    case 'ecommerce-get' :
                        $this->assertEcommerceGet($result['data']);

                        break;
                    case 'ecommerce-post' :
                        $this->assertEcommercePost($result['data']);
    ...
    private function assertPageViewGet($data): void
    {
    ...
    private function assertPageViewPost($data): void
    {
        $data = json_decode($data, true);
    ...
    private function assertScreenViewGet($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('eid', $data);
        $this->assertArrayHasKey('dtm', $data);
    ...
    private function assertScreenViewPost($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('schema', $data);
        $this->assertArrayHasKey('data', $data);
    ...
    private function assertStructEventGet($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('eid', $data);
        $this->assertArrayHasKey('dtm', $data);
    ...
    private function assertStructEventPost($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('schema', $data);
        $this->assertArrayHasKey('data', $data);
    ...
    private function assertUnStructEventGet($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('eid', $data);
        $this->assertArrayHasKey('dtm', $data);
    ...
    private function assertUnStructEventPost($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('schema', $data);
        $this->assertArrayHasKey('data', $data);
    ...
    private function assertEcommerceGet($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('eid', $data);
        $this->assertArrayHasKey('dtm', $data);
    ...
    private function assertEcommercePost($data): void
    {
        $data = json_decode($data, true);
        $this->assertArrayHasKey('schema', $data);
        $this->assertArrayHasKey('data', $data);
